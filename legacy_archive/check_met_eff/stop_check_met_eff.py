...
if len(sys.argv) != 3:
    print("Usage: python stop_check_BR_branches_per_event.py /path/to/input_file.root /path/to/output_file.json")
    sys.exit(1)

input_file = sys.argv[1]
output_file = sys.argv[2]
n_processed_events = 2000
...
num_mases_dict = {}
for masspoint in known_masspoints:
    num_mases_dict[masspoint] = [0.0, 0.0]
    num_mases_dict[masspoint + "_passing_MET"] = [0.0, 0.0]
    num_mases_dict[masspoint + "_failing_MET"] = [0.0, 0.0]
    num_mases_dict[masspoint + "_min_MET_pt"] = [10000000.0, 10000000.0]
num_mases_dict["nEventsTotal"] = 0.0
num_mases_dict["strange masspoints"] = 0.0
num_mases_dict["orphans"] = 0.0
num_mases_dict["R-hadron event"] = 0.0
num_mases_dict["different stop antistop masses"] = 0.0
num_mases_dict["strange error"] = 0.0
num_mases_dict["strange event"] = 0.0
num_mases_dict["different neutralino masses"] = 0.0
...
stop_pdgid = 1000006
X0_pdgid = 1000022
r_hadron_pdgid = [1000612,1000622,1000632,1000642,1000652]
...
for entry in tree:
    ...
    for i, pdgId in enumerate(entry.GenPart_pdgId):
        ...
        if abs(pdgId) in r_hadron_pdgid:#This is checking for r hadrons
            print("R-hadrond found")
            r_hadron = True
        ...
        if abs(pdgId) == X0_pdgid:
            ...
            if entry.GenPart_pdgId[mother_x0_idx] == stop_pdgid:
                ...
                X0_generated_from_stop_idx = i

            if entry.GenPart_pdgId[mother_x0_idx] == -stop_pdgid:
                stop_anti_decaying_idx = mother_x0_idx
                X0_generated_from_anti_stop_idx = i
        ...
        if mother_idx_i!= -1 and entry.GenPart_pdgId[mother_idx_i] == stop_pdgid and pdgId != stop_pdgid and i not in decay_from_stop_idx:
            ...
            if mother_status_i == 106:
                ...
                # if decay_from_stop_pdgid == abs(X0_pdgid):
                #     X0_generated_from_stop_idx = i
        ...
        if mother_idx_i!= -1 and entry.GenPart_pdgId[mother_idx_i] == -stop_pdgid and pdgId != -stop_pdgid and i not in decay_from_anti_stop_idx:
            ...
            if mother_status_i == 106:
                ...
                # if decay_from_anti_stop_pdgid == abs(X0_pdgid):
                #     X0_generated_from_anti_stop_idx = i

    #Error handling
    if r_hadron:
        num_mases_dict["R-hadron event"] += 1
        continue
    ...
    try:
        # print("stop_generated_idx = "+str(stop_generated_idx))
        # print("stop_anti_generated_idx = "+str(stop_anti_generated_idx))
        # print("X0_generated_from_stop_idx = "+str(X0_generated_from_stop_idx))
        # print("X0_generated_from_anti_stop_idx = "+str(X0_generated_from_anti_stop_idx))
        
        mass_stop = entry.GenPart_mass[stop_generated_idx]
        mass_anti_stop = entry.GenPart_mass[stop_anti_generated_idx]
        mass_X0 = entry.GenPart_mass[X0_generated_from_stop_idx]
        mass_X0_from_anti_stop = entry.GenPart_mass[X0_generated_from_anti_stop_idx]
        
        # print("mass_stop = "+str(mass_stop))
        # print("mass_anti_stop = "+str(mass_anti_stop))
        # print("mass_X0 = "+str(mass_X0))
        # print("mass_X0_from_anti_stop = "+str(mass_X0_from_anti_stop))
        # print(".........................................................")
    except:
    ...
    if mass_X0_from_anti_stop != mass_X0:
        # print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        # print("DIFFERENT NEUTRALINO MASES")
        # print("mass_X0_from_anti_stop = "+str(mass_X0_from_anti_stop))
        # print("mass_X0 = "+str(mass_X0))
        # print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        num_mases_dict["different neutralino masses"] += 1 
    ...
    if (mass_stop not in known_mStop) or (mass_X0 not in known_mX0) or (masspoint_key not in known_masspoints):
        ...
        num_mases_dict[new_masspoint+"_passing_MET"] = [0.0,0.0]
        num_mases_dict[new_masspoint+"_failing_MET"] = [0.0,0.0]
        num_mases_dict[new_masspoint+"_min_MET_pt"] = [10000000.0, 10000000.0]
    ...
    if masspoint_key in known_masspoints_original:
        ...
        if BR_03_found and BR_10_found:
            ...
            if BR_03_value:
                num_mases_dict[masspoint_key][0] += 1
                #Evaluate if MET value meet cut criteria    
                if event_met > met_cut:
                    num_mases_dict[masspoint_key+"_passing_MET"][0] += 1
                else:
                    num_mases_dict[masspoint_key+"_failing_MET"][0] += 1
                if event_met < num_mases_dict[masspoint_key+"_min_MET_pt"][0]:
                    num_mases_dict[masspoint_key+"_min_MET_pt"][0] = event_met
        elif BR_10_found:
            if BR_10_value:
                num_mases_dict[masspoint_key][1] += 1
                #Evaluate if MET value meet cut criteria    
                if event_met > met_cut:
                    num_mases_dict[masspoint_key+"_passing_MET"][1] += 1
                else:
                    num_mases_dict[masspoint_key+"_failing_MET"][1] += 1
                if event_met < num_mases_dict[masspoint_key+"_min_MET_pt"][1]:
                    num_mases_dict[masspoint_key+"_min_MET_pt"][1] = event_met
        else:
            do_seach = True
    else:
        do_seach = True
