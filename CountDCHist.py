...
if 'T2tt' in samples:
    ...
    for b in range(bins): histos['h_rate'].GetXaxis().SetBinLabel(b+1, binLabel[b])
    ...
    for ientry in range(n_entries):
        ...
        if region == 'CR':
            ...
            if getsel.CR2():
                ...
                if not idx == 7: histos['h_rate'].Fill(idx, lumiscale * MCcorr)
            if getsel.CR3():
                idx = findCR2BinIndex(getsel.calCT(2), getsel.getLepMT()) + 16
                if not idx == 15: histos['h_rate'].Fill(idx, lumiscale * MCcorr)
        if region == 'SR+CR':
            ...
            if getsel.ControlRegion():
                if getsel.CR1():
                    ...
                    if not idx == 107: histos['h_rate'].Fill(idx, lumiscale * MCcorr)
                if getsel.CR2():
                    idx = findCR2BinIndex(getsel.calCT(2), getsel.getLepMT()) +  108 + 8
                    if not idx == 115: histos['h_rate'].Fill(idx, lumiscale * MCcorr)
                if getsel.CR3():
                    idx = findCR2BinIndex(getsel.calCT(2), getsel.getLepMT()) + 116 + 8
                    if not idx == 123: histos['h_rate'].Fill(idx, lumiscale * MCcorr)
