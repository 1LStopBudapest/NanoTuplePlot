...
from FillHistos_LL_std import FillHistos
...
if isinstance(samplelist[samples][0], types.ListType):
    ...
    for s in samplelist[samples]:
        ...
        hfile = ROOT.TFile( 'StackHist_'+sample+'_%i_%i'%(options.startfile+1, options.startfile + options.nfiles)+'_std.root', 'RECREATE')
        histos = {}
