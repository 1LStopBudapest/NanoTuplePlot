...
class FillHistosBothBR():
    ...
    def __init__(self, hfile, histos, chain03, chain10, year, nEvents, sample, vList, branching_ratio, DataLumi=1.0, NoMCWeight = True, eletype = 'comb'):
        self.histos = histos
    ...
    def fill2(self):
        ...
        for ientry03 in range(n_entries03):
            ...
            getsel03 = TreeVarSel(tree03, self.isData, self.year, self.eletype)
            if getsel03.passFilters() and getsel03.PreSelection():
                ...
                if getsel03.getSortedLepVar()[0]['type'] == 'mu':
                    ...
                    var03['Mudxy'] = abs(getsel03.getSortedLepVar()[0]['dxy'])
                    var03['Mudz'] = abs(getsel03.getSortedLepVar()[0]['dz'])
                else:
                    var03['epT'] = getsel03.getSortedLepVar()[0]['pt']
                    var03['edxy'] = abs(getsel03.getSortedLepVar()[0]['dxy'])
                ...
                if len(getsel03.getSortedLepVar()) > 1:
                    var03['2ndLeppT'] = getsel03.getSortedLepVar()[1]['pt']
                    var03['2ndLepeta'] = abs(getsel03.getSortedLepVar()[1]['eta'])
                    var03['2ndLepdxy'] = abs(getsel03.getSortedLepVar()[1]['dxy'])
                    var03['2ndLepdz'] = abs(getsel03.getSortedLepVar()[1]['dz'])
                #############
        ...
        for ientry10 in range(n_entries10):
            ...
            getsel10 = TreeVarSel(tree10, self.isData, self.year, self.eletype)
            tail_event_10 = False
            ...
            if getsel10.passFilters() and getsel10.PreSelection():
                ...
                if getsel10.getSortedLepVar()[0]['type'] == 'mu':
                    ...
                    var10['Mudxy'] = abs(getsel10.getSortedLepVar()[0]['dxy'])
                    var10['Mudz'] = abs(getsel10.getSortedLepVar()[0]['dz'])
                else:
                    var10['epT'] = getsel10.getSortedLepVar()[0]['pt']
                    var10['edxy'] = abs(getsel10.getSortedLepVar()[0]['dxy'])
                ...
                if len(getsel10.getSortedLepVar()) > 1:
                    var10['2ndLeppT'] = getsel10.getSortedLepVar()[1]['pt']
                    var10['2ndLepeta'] = abs(getsel10.getSortedLepVar()[1]['eta'])
                    var10['2ndLepdxy'] = abs(getsel10.getSortedLepVar()[1]['dxy'])
                    var10['2ndLepdz'] = abs(getsel10.getSortedLepVar()[1]['dz'])
        ...
        #The csv rows are built from one fixed reference variable, not from whatever
        #histogram happened to come last in the dictionary iteration above
        key_csv = 'MET'

        print("#####################################################")
        print("reference variable for the csv rows: "+str(key_csv))
        print("nentries_03.append("+ str(histos03[key_csv].GetEntries()) +")")
        print("nentries_10.append("+ str(histos10[key_csv].GetEntries()) +")")
        print("nentries_10_tail.append("+ str(histos10_tail[key_csv].GetEntries()) +")")
        print("nentries_combined.append("+ str(self.histos[key_csv].GetEntries()) +")")

        print("integral_03.append("+ str(histos03[key_csv].Integral()) +")")
        print("integral_10.append("+ str(histos10[key_csv].Integral()) +")")
        print("integral_combined.append("+ str(self.histos[key_csv].Integral()) +")")
        print("integral_tail.append("+ str(histos10_tail[key_csv].Integral()) +")")
        ...
        row_csv_10 = [self.ms,
                      ...
                      histos10[key_csv].GetEntries(),
                      n_rejected_10,
                      n_4bd4bd_10,
                      n_4bd2bd_10,
                      n_2bd2bd_10,
                      histos10[key_csv].Integral()]
        row_csv_03 = [self.ms,
                      self.ml,
                      self.branching_ratio,
                      histos03[key_csv].GetEntries(),
                      n_rejected_03,
                      n_4bd4bd_03,
                      n_4bd2bd_03,
                      n_2bd2bd_03,
                      histos03[key_csv].Integral()]
        row_csv_combined = [self.ms,
                            self.ml,
                            self.branching_ratio,
                            self.histos[key_csv].GetEntries(),
