...
output_folder = "/afs/cern.ch/work/m/mleoncoe/private/stop_met_check/outputs_2018/"

runOn=[input_folder+f for f in os.listdir(input_folder)]
tmp_condor = open('jobs_2018/submitFile_stop_met_check.condor', 'w')
tmp_condor.write('''Executable = execute_stop_met_check.sh
getenv      = True
Log        = jobs_2018/log_running_$(ProcId).log
Output     = jobs_2018/log_running_$(ProcId).out
Error      = jobs_2018/log_running_$(ProcId).error
Transfer_Input_Files = stop_check_met_eff.py
arguments  = $(info)
...
for ifile in runOn:
    ...
    tmp_condor.write('stop_check_met_eff.py {input} {output} \n'.format(input=ifile,output=output_folder+file_name_no_extension+".json"  ) )

tmp_condor.write(') \n')
tmp_condor.close()

print('condor_submit jobs_2018/submitFile_stop_met_check.condor')