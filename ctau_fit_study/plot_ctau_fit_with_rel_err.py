...
if len(sys.argv) != 5:
    print "Usage: python plot_ctau_fit.py combined? (combined, low, high) mstop mX0 BR"
    sys.exit(1)

is_combined = sys.argv[1]
mstop = sys.argv[2]
mX0 = sys.argv[3]
BR_reweighted = sys.argv[4]
...
if is_combined == "combined":
    filepath = "/home/mleoncoe/stopAnalysis/test/Plots/1DFiles_LL/2016PostVFP/BR_"+BR_reweighted+"/1DHist_Sig_Splitted_"+str(mstop)+"_"+str(mX0)+"_1_100.root"
    print(filepath)
    # Open the ROOT file
    file = ROOT.TFile.Open(filepath)
    hist = file.Get("stopCtau_Sig_Splitted_300_280")
elif is_combined == "low":
    filepath = "/home/mleoncoe/stopAnalysis/test/Plots/1DFiles_LL/2016PostVFP/BR_"+BR_reweighted+"/1DHist_Sig_Splitted_"+str(mstop)+"_"+str(mX0)+"_BR_0p300_1_1000.root"
    print(filepath)
    # Open the ROOT file
    file = ROOT.TFile.Open(filepath)
    hist = file.Get("stopCtau_Sig_Splitted_300_280_BR_0p300")
elif is_combined == "high":
    filepath = "/home/mleoncoe/stopAnalysis/test/Plots/1DFiles_LL/2016PostVFP/BR_"+BR_reweighted+"/1DHist_Sig_Splitted_"+str(mstop)+"_"+str(mX0)+"_BR_1p000_1_1000.root"
    print(filepath)
    # Open the ROOT file
    file = ROOT.TFile.Open(filepath)
    hist = file.Get("stopCtau_Sig_Splitted_300_280_BR_1p000")
else:
    print("invalid first option")
    exit()


# Check if histogram was loaded successfully
if not hist:
    print("Histogram not found!")
    exit()

# Define exponential function: f(x) = [1] * exp(-x / [0])
fit_func = ROOT.TF1("fit_pdf", "[1] * exp(-x / ([0]))", 0.01, hist.GetXaxis().GetXmax())

# Set initial guesses for parameters
fit_func.SetParameter(0, float(BR_targets_dic[BR_reweighted]) )  # tau (decay constant)
fit_func.SetParameter(1, hist.GetMaximum())  # normalization


# Fit the histogram
fit_result = hist.Fit(fit_func, "RS")  # "R" = fit in specified range
# Extract fitted ctau value and uncertainty
fitted_ctau = fit_func.GetParameter(0)
fitted_ctau_err = fit_func.GetParError(0)
fitted_amp = fit_func.GetParameter(1)
fitted_amp_err = fit_func.GetParError(1)
...
true_ctau_str = " TrueCtau = " + BR_targets_dic[BR_reweighted]

########1
# Create canvas to draw
#canvas = ROOT.TCanvas("canvas", "Exponential Fit", 600, 1000)
#canvas.SetLogy()

# Create canvas with two pads
canvas = ROOT.TCanvas("canvas", "Exponential Fit with Relative Errors", 600, 800)
canvas.Divide(1, 2)  # Divide canvas into 2 vertical pads
# Upper pad: original histogram
canvas.cd(1)
upper_pad = canvas.GetPad(1)
upper_pad.SetPad(0, 0.3, 1, 1)  # xlow, ylow, xup, yup
upper_pad.SetBottomMargin(0.05)     # Remove bottom margin
upper_pad.SetTopMargin(0.1)
upper_pad.SetLogy()
upper_pad.Draw()
...
# Draw histogram
hist.Draw("HISTE")  # Draw with error bars

# Draw fit function
fit_func.SetLineColor(ROOT.kRed)
fit_func.Draw("same")

# Create a TPaveText to show fit parameters
pave = ROOT.TPaveText(0.6, 0.7, 0.88, 0.88, "NDC")
pave.SetFillColor(0)
pave.SetBorderSize(1)

# Add fit results as text
pave.AddText("Fit Results:")
pave.AddText("ctau = {:.3f}".format(fit_func.GetParameter(0)))
pave.AddText("norm = {:.3f}".format(fit_func.GetParameter(1)))
pave.AddText(true_ctau_str)
pave.AddText(fitted_ctau_FOM_str)
...
###########################
# --- Create ratio histogram for errors/content ---
h_ratio = hist.Clone("h_ratio")
h_ratio.Reset("ICES")  # clear contents, keep binning
...
for i in range(1, hist.GetNbinsX()+1):
    ...
    h_ratio.SetBinError(i, 0)  # usually don't plot error on error/content

h_ratio.SetTitle("")
h_ratio.GetYaxis().SetTitle("Error / Content")
...
h_ratio.SetStats(0)  # This removes the statistics box
##########################
...
canvas.SaveAs("stop_ctau_"+BR_reweighted+"_masses_"+str(mstop)+"_"+str(mX0)+"_"+is_combined+"_.png")
