...
def get_parser():
    ...
    dest='alist',                           # store in 'list'.
        default=['VV', 'TTV', 'ZJetsToNuNu', 'QCD', 'DYJetsToLL', 'ST', 'TTbar', 'WJetsToLNu', 'MET_Data'],     # last sample should be data (when data is included) as to be consistent with StackHists funtion.
    )
...
for sl in samplelists:
    if os.path.exists('PromptBKVal2_'+reg+'_'+sl+'.root'):
        files.append(ROOT.TFile.Open('PromptBKVal2_'+reg+'_'+sl+'.root'))
    elif os.path.exists(plotDir+filedir+'/PromptBKVal2_'+reg+'_'+sl+'.root'):
        files.append(ROOT.TFile.Open(plotDir+filedir+'/PromptBKVal2_'+reg+'_'+sl+'.root'))
    else:
        doplots = False        
