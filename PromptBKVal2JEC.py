...
if 'T2tt' in samples:
    ...
    hfile = ROOT.TFile( 'PromptBKVal2JEC_'+region+'_'+year+'_'+sample+'_%i_%i'%(options.startfile+1, options.startfile + options.nfiles)+'.root', 'RECREATE')
    histos = {}
    ...
    for ientry in range(n_entries):
        ...
        for tp in ['Nom', 'JECUp', 'JECDown', 'JERUp', 'JERDown']:
            ...
            if region == 'SR+CR':
                if getsel.Val2SearchRegion(tp):
                    if getsel.Val2SR1(tp):
                        idx = findSR1BinIndexVal2(getsel.calCT(1, tp), getsel.getLepMT(), getsel.getSortedLepVar()[0]['pt'], getsel.getSortedLepVar()[0]['charg'])
                        if not idx == -1:
                            h.Fill(idx, lumiscale * MCcorr)
                    if getsel.Val2SR3(tp):
                        idx = findSR3BinIndexVal2(getsel.calCT(2, tp), getsel.getLepMT(), getsel.getSortedLepVar()[0]['pt']) + 36
                        if not idx <= 35:
                            h.Fill(idx, lumiscale * MCcorr)
                if getsel.Val2ControlRegion(tp):
                    if getsel.Val2CR1(tp):
                        idx = findCR1BinIndexVal2(getsel.calCT(1, tp), getsel.getLepMT(), getsel.getSortedLepVar()[0]['charg']) + 72 # after 72 SR bins or after bin index 71 
                        if not idx <= 71:
                            h.Fill(idx, lumiscale * MCcorr)
                    if getsel.Val2CR3(tp):
                        idx = findCR3BinIndexVal2(getsel.calCT(2, tp), getsel.getLepMT()) + 72 + 8
                        if not idx <= 79:
                            h.Fill(idx, lumiscale * MCcorr)
            ...
            for ientry in range(n_entries):
                ...
                for tp in ['Nom', 'JECUp', 'JECDown', 'JERUp', 'JERDown']:
                    ...
                    if region == 'SR+CR':
                        ...
                        if getsel.Val2ControlRegion(tp):
                            if getsel.Val2CR1(tp):
                                idx = findCR1BinIndexVal2(getsel.calCT(1, tp), getsel.getLepMT(), getsel.getSortedLepVar()[0]['charg']) + 72 # after 24 SR bins or after bin index 23 
                                if not idx <= 71:
                                    h1.Fill(idx, lumiscale * MCcorr)
                                    if promptFlag: h2.Fill(idx, lumiscale * MCcorr)
                                    else: h3.Fill(idx, lumiscale * MCcorr)
