...
# ===================================================================
# Helper functions: classify where a generated lepton comes from
# inside the stop decay chain
# ===================================================================
...
def get_stop_ancestry(gen_lep, tree, stop_pdgId=1000006, max_steps=200, print_parents=False):
    """
    Walks upwards through mother indices and collects the pdgId of every
    ancestor that is crossed on the way up.
    Returns (stop_pdg, chain), where stop_pdg is the SIGNED pdgId of the stop
    the walk ended on (+1000006 stop, -1000006 anti-stop), or 0 if no stop was
    reached, and chain is the list of pdgIds that were crossed.
...
def leg_is_2body(stop_pdg, tree):
    """
...
def classify_lepton_origin(gen_lep, tree, stop_pdgId=1000006, max_steps=200):
    """
    Classifies where a generated lepton comes from inside the stop decay chain.
    Returns one of the keys of ORIGIN_CODES, testing in this order:
    ...
    """
    stop_pdg, chain = get_stop_ancestry(gen_lep, tree, stop_pdgId, max_steps)

    if stop_pdg == 0:
        return 'not_from_stop'

    if leg_is_2body(stop_pdg, tree):
        return 'stop_2body'
    ...
    def fill2(self):
        ...
        for ientry03 in range(n_entries03):
            ...
            if True:
                ...
                for gen_lep in final_leptons:
                    gen_lep["origin"] = classify_lepton_origin(gen_lep, tree03)
                ...
                #     for gen_lep in final_leptons:
                #         if gen_lep["fromStop"]:
                #             dumb = get_stop_ancestry(gen_lep, tree03, print_parents=True)
                ############################################################
                ...
                if len(final_leptons)>=1:
                    ...
                    #######################################
                    if truth_matched_lepton and matched_gen_lep is not None:
                        var03["2Dpt"] = []
        ...
        for ientry10 in range(n_entries10):
            ...
            if True:
                ...
                for gen_lep in final_leptons:
                    gen_lep["origin"] = classify_lepton_origin(gen_lep, tree10)
                    gen_lep["fromStop"] = gen_lep["origin"] != 'not_from_stop'
        ...
        append_row_to_csv(os.path.join(_HERE, 'info_test_leptonTruth_10.csv'), row_csv_10)
        append_row_to_csv(os.path.join(_HERE, 'info_test_leptonTruth_03.csv'), row_csv_03)
        append_row_to_csv(os.path.join(_HERE, 'info_test_leptonTruth_combined.csv'), row_csv_combined)
