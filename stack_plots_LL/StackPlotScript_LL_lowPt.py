    ...
    if isinstance(samplelist[sL][0], types.ListType):
        ...
        for s in samplelist[sL]:
            ...
            for i in range(0, tfiles, fileperjobMC):
                #txtline.append("python StackHistMaker_LL_lowPt.py --sample %s --startfile %i --nfiles %i\n"%(sample, i, fileperjobMC))
                #Moises
                processed_samples_i = processed_samples_i+1
                if processed_samples_i > nsamples: continue
                txtline.append("python StackHistMaker_LL_lowPt.py --sample %s --nevents %i --startfile %i --nfiles %i\n"%(sample, nevents, i, fileperjobMC))
                ###
            #Moises
            processed_samples_i = 0
    else:
...
fout = open("parallelJobsubmit_lowPt.txt", "w")
...
bashline = []    
bashline.append('parallel --jobs %i < parallelJobsubmit_lowPt.txt\n'%TotJobs)

for sL in samplesRun:

    if 'Data' in sL:
        sLi = sL.replace('Data','')+'Run'
        bashline.append('hadd StackHist_%s_lowPt.root StackHist_%s*_lowPt.root\n'%(sL, sLi))
        ...
        #Moises
        sLi = 'hadd StackHist_' + sL + '_lowPt.root'
        file_patterns = []
        for s in samplelist[sL]:
            #Moises
            ...
            file_pattern = ' StackHist_' + sample_key + '*_lowPt.root'
        ...
        bashline.append('hadd StackHist_%s_lowPt.root StackHist_%s_*_lowPt.root\n'%(sL, sL))

    #bashline.append('mv StackHist_%s.root %s\n'%(sL, Rootfilesdirpath))
    #Moises
    bashline.append('mv StackHist_%s_lowPt.root %s\n'%(sL, Rootfilesdirpath_added))
    #####

bashline.append('mv StackHist_*_lowPt.root %s\n'%(Rootfilesdirpath))

l = str(" ".join(s for s in samplesRun))
bashline.append('python  StackPlot_LL_lowPt.py -l %s'%l)
    
fsh = open("parallelStackHist_lowPt.sh", "w")
fsh.write(''.join(bashline))
fsh.close()
os.system('chmod 744 parallelStackHist_lowPt.sh')
os.system('./parallelStackHist_lowPt.sh')
#os.system('rm *.root parallelJobsubmit.txt parallelStackHist_lowPt.sh')
